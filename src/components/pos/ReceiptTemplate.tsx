...
import { CartItem, PaymentBreakdown, PricingRule, SalesTransaction } from '@/types';
import { pricingEngine, PriceCalculation } from '@/services/pricing/PricingEngine';
import { receiptTemplateService } from '@/services/receipt/ReceiptTemplateService';
...
export interface ReceiptItem extends CartItem {
  pricingCalculation?: PriceCalculation;
...
const ReceiptTemplate: React.FC<ReceiptTemplateProps> = ({
  ...
  const renderReceiptHeader = () => (
    <div className="text-center mb-4 border-b border-dashed border-gray-400 pb-3">
      {showLogo && (
        <div className="mb-2">
          <h1 className={`font-bold ${getFontSizeClass()} text-gray-900`}>
            POS PWA RETAIL
          </h1>
      ...
      <div className={`mt-3 text-gray-700 ${getFontSizeClass()}`}>
        ...
        <p>{formatDate(data.date)} {formatTime(data.date)}</p>
        {showCashier && (
  ...
  const renderItems = () => (
    <div className="mb-4">
      {/* Table Header */}
      <div className={`grid grid-cols-12 gap-1 font-bold border-b border-gray-300 pb-1 ${getFontSizeClass()}`}>
      ...
      <div className="space-y-1">
        {data.items.map((item, index) => (
          <div key={`${item.itemId}-${index}`} className={`grid grid-cols-12 gap-1 py-1 border-b border-gray-100 ${getFontSizeClass()}`}>
  ...
  const renderTaxBreakdown = () => (
    <div className="mb-4">
      ...
      <div className={`flex justify-between font-bold border-t border-gray-200 pt-1 mt-1 ${getFontSizeClass()}`}>
        <span>Total Tax:</span>
  ...
  const renderPaymentDetails = () => {
    const totalPayment = data.payments.cash + data.payments.card + data.payments.ewallet + data.payments.bankTransfer + data.payments.credit;
    ...
    return (
      <div className="mb-4 border-t border-dashed border-gray-400 pt-3">
        ...
        <div className={`flex justify-between font-medium mt-2 pt-2 border-t border-gray-200 ${getFontSizeClass()}`}>
  ...
  const renderTotals = () => (
    <div className="border-t border-dashed border-gray-400 pt-3">
      <div className={`space-y-1 ${getFontSizeClass()}`}>
        ...
        <div className={`flex justify-between font-bold text-lg border-t border-gray-400 pt-1 ${getFontSizeClass()}`}>
          <span>TOTAL:</span>
  ...
  const handlePrint = async () => {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Thermal printer not available, falling back to browser print:', printerError);
...
export const generateReceiptData = (
  ...
  const receiptNumber = `RCP-${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}-${now.getTime().toString().slice(-6)}`;
  ...
  const paymentBreakdown = {
    ...
    totalPayment: payment.cash + payment.card + payment.ewallet + payment.bankTransfer + payment.credit
  };
