...
import { receiptTemplateService as _receiptTemplateService } from './ReceiptTemplateService';
...
export class ESCPosPrinter {
   ...
   */
  async printReceipt(transaction: SalesTransaction, printerName?: string): Promise<string> {
    try {
  ...
  private generateTotalsCommands(transaction: SalesTransaction): string[] {
    ...
    // Subtotal
    commands.push(
      `${'Subtotal:'.padEnd(this.config.width - 10) + transaction.subtotalAmount.toFixed(2)}\n`
    );
    ...
    if (transaction.discountAmount > 0) {
      ...
      commands.push(`${'-'.repeat(this.config.width)}\n`);
      commands.push(
        `${'Total Discount:'.padEnd(this.config.width - 10)}-${transaction.discountAmount.toFixed(
          2
        )}\n`
      );
    }
    ...
    if (transaction.taxAmount > 0) {
      commands.push(
        `${'Tax:'.padEnd(this.config.width - 10) + transaction.taxAmount.toFixed(2)}\n`
      );
    }
    ...
    commands.push('\x1B\x21\x10'); // Double height
    commands.push(
      `${'TOTAL:'.padEnd(this.config.width - 10) + transaction.totalAmount.toFixed(2)}\n`
    );
    commands.push('\x1B\x21\x00'); // Normal size
  ...
  private generatePaymentCommands(transaction: SalesTransaction): string[] {
    ...
    if (transaction.paymentMethod) {
      ...
      if (transaction.paymentMethod.change && transaction.paymentMethod.change > 0) {
        commands.push(
          `${
            'Change:'.padEnd(this.config.width - 10) + transaction.paymentMethod.change.toFixed(2)
          }\n`
        );
      }
    }
