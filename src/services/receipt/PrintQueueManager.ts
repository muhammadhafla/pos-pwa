...
export class PrintQueueManager {
  ...
  /**
   * Initialize print queue from database
   */
  private async initializeFromDatabase(): Promise<void> {
    try {
      await this.loadPrintJobsFromDatabase();
      console.log('📄 Print queue initialized from database');
    } catch (error) {
      console.error('Failed to initialize print queue from database:', error);
    }
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    const activePrintJobs = Array.from(this.printQueue.values()).filter(
   ...
   */
  private async printWithBrowser(job: PrintJob, _commands: number[]): Promise<boolean> {
    try {
          ...
          <div>
            ...
            ${
              item.discount && item.discount > 0
                ? `<div>Diskon: -${formatCurrency(item.discount)}</div>`
                : ''
            }
          </div>
        `
        ...
        <div style="margin: 20px 0; text-align: center;">
          ${
            receiptData.footer.thankYouMessage
              ? `<div style="font-weight: bold;">${receiptData.footer.thankYouMessage}</div>`
              : ''
          }
  ...
  private async handlePrintFailure(job: PrintJob, error: Error): Promise<void> {
    ...
    if (job.attempts < job.maxAttempts) {
      // Schedule retry
      const retryDelay =
        this.RETRY_DELAYS[Math.min(job.attempts - 1, this.RETRY_DELAYS.length - 1)];
  ...
  private async savePrintJobToDatabase(job: PrintJob): Promise<void> {
    try {
      // Convert PrintJob to PrintJobRecord format
      ...
      const existingJob = await db.printJobs.get(job.id);

      if (existingJob) {
        // Update existing job
        await db.updatePrintJobStatus(job.id, job.status, job.errorMessage);
      } else {
        // Create new job
        await db.createPrintJob(printJobRecord);
      }

      console.log('✅ Print job saved to database:', job.id);
    } catch (error) {
  ...
  async loadPrintJobsFromDatabase(): Promise<void> {
    try {
      // Load all print jobs from database
      const dbPrintJobs = await db.printJobs.toArray();

      // Convert database records back to PrintJob format and add to queue
      for (const dbJob of dbPrintJobs) {
        const job: PrintJob = {
          id: dbJob.id,
          transactionId: dbJob.transactionId,
          receiptData: dbJob.receiptData,
          priority: dbJob.priority,
          status: dbJob.status,
          attempts: dbJob.attempts,
          maxAttempts: dbJob.maxAttempts,
          createdAt: dbJob.createdAt,
          lastAttemptAt: dbJob.lastAttemptAt,
          errorMessage: dbJob.errorMessage,
          printerName: dbJob.printerName,
        };

        // Only add jobs that are not permanently failed or completed
        if (job.status !== 'failed' || job.attempts < job.maxAttempts) {
          this.printQueue.set(job.id, job);
        }
      }

      console.log(`✅ Loaded ${dbPrintJobs.length} print jobs from database`);
    } catch (error) {
    ...
    for (const [jobId, job] of this.printQueue.entries()) {
      if (
        ...
        try {
          ...
          console.log('✅ Deleted print job from database:', jobId);
        } catch (error) {
          console.error('Failed to delete print job from database:', jobId, error);
        }
      }
    }
  }
