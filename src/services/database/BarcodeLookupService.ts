...
export class BarcodeLookupService {
  ...
  private async initializeIndexes(): Promise<void> {
    try {
      ...
      console.log(`✅ Barcode indexes initialized: ${items.length} items, ${this.barcodeIndex.size} barcodes (${initTime.toFixed(2)}ms)`);
  ...
  async bulkLookup(barcodes: string[]): Promise<BarcodeLookupResult[]> {
    ...
    console.log(`✅ Bulk lookup completed: ${totalTime.toFixed(2)}ms total, ${avgTime.toFixed(2)}ms average`);
