...
export class BarcodeScannerManager {
  ...
  private keypressTimeout: number | null = null;
  ...
  private async setupUSBDevice(device: any): Promise<void> {
    try {
      ...
      const interfaceIndex = device.configuration.interfaces.findIndex((iface: any) =>
        iface.alternates.some((alt: any) => alt.interfaceClass === 3)
      );
  ...
  private isBarcodeScanner(device: any): boolean {
    ...
    return (
      scannerVIDs.includes(device.vendorId) ||
